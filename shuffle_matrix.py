...
def swap_shuffle(matrix: Matrix, original_matrix: Matrix) -> tuple[Matrix, float]:
  ...
  score = score_matrix(original_matrix, matrix)
...
def training(matrix: Matrix, original_matrix: Matrix, n_steps: int, alpha: float=0.99, T=1.0, debug_steps: int=0) -> None:
  ...
  for i in range(n_steps):
    new_matrix, new_score = swap_shuffle(matrix=old_matrix, original_matrix=original_matrix)
    ...
    if new_score > current_score:
      ...
      if random.uniform(0, 1) < np.exp(-dE/T):
    ...
    if i & debug_steps:
      if i % debug_steps == 0:
        print(f"Score on iteration {i+1} = {current_score:.4f} | Value of T {T}")
